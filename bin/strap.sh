...
PUB_KEY_IDENTIFIER=$USER@`hostname -s`

rm github_rsa github_rsa.pub

ssh-keygen -t rsa -b 4096 -C "$PUB_KEY_IDENTIFIER" -N "" -f ./github_rsa -q

# Todo: read it, and username and password too
TFA=015677

# Todo: rm public key token with exact name if exists
TOKEN=$(curl -s -u "MeddahJ:UrMom" -H "X-GitHub-OTP:$TFA" "https://api.github.com/authorizations" -d "{
  \"scopes\": [
    \"write:public_key\",
    \"user:email\"
  ],
  \"note\": \"[$(hostname -s)] Write public keys, read user data (`date`)\"
}" | python -c 'import sys, json; print json.load(sys.stdin)[sys.argv[1]]' token) # jQ not available at this time


echo "Public key token $TOKEN created"

curl -s -u "MeddahJ:$TOKEN" "https://api.github.com/user/keys" -d "{
  \"title\": \"Public key for $PUB_KEY_IDENTIFIER generated by Strap\",
  \"key\": \"`cat github_rsa.pub`\"
}"
...
## In non-sudo mode, attempt to set homebrew_prefix inside /Users/Guest or where appropriate
